++ b/backend/services/tituladas_service.py
# FECHAS Y HORAS (d├¡as h├íbiles lunes-viernes ├ù bloque de 6 h)
...
async def listar_fichas(buscar: str | None = None, jornada: str | None = None, sede: str | None = None) -> list:
    """Listado de fichas tituladas con su % de programaci├│n calculado."""
    if not _es_demo():
        ...
        # Opcional: Podr├¡amos traer instructores para llenar el titular,
        # pero para el listado b├ísico podemos mapear los campos.
        fichas_mapeadas = []
        for f in fichas_db:
            # Filtros manuales si vienen en la petici├│n (se podr├¡an hacer en OData tambi├®n)
            ...
            fichas_mapeadas.append({
                ...
                "codigo": codigo,
                "programa": programa,
                "nivel": "T├®cnico", # Valor por defecto si no lo tenemos en la tabla ficha
                "jornada": {430120000: "Ma├▒ana", 430120001: "Tarde", 430120002: "Noche"}.get(f.get("cr6a3_jornada"), "Ma├▒ana"),
...
async def obtener_ficha(ficha_id: str) -> dict:
    """Detalle de la ficha: diagn├│stico de competencias + asignaciones del calendario."""
    if not _es_demo():
        from services.dataverse import consultar_dataverse
        ...
        for c in competencias_db:
            diagnostico.append({
                "id": c.get("cr6a3_competenciafichaid"),
                "nombre": c.get("cr6a3_nombre", ""),
                "tipo": c.get("cr6a3_tipo", ""),
                "horas": c.get("cr6a3_horas", 0),
                "horas_programadas": 0, # Pendiente motor de asignaciones
                "porcentaje": 0,
                "instructores": [],
            })
            
        # Extraer ID din├ímico por si Dataverse lo llama cr6a3_fichasid o cr6a3_fichaid
        ficha_id_key = next((k for k in ficha_db.keys() if k.startswith("cr6a3_") and k.endswith("id")), "cr6a3_fichaid")
        
        map_j = {430120000: "Ma├▒ana", 430120001: "Tarde", 430120002: "Noche"}
        ...
        return {
            ...
            "nivel": "T├®cnico",
            "jornada": map_j.get(ficha_db.get("cr6a3_jornada"), "Ma├▒ana"),
            ...
            "archivos": [],
            "asignaciones": [], # Pendiente motor de asignaciones
            "total_horas_programa": sum(c["horas"] for c in diagnostico),
            "horas_programadas": 0,
            "porcentaje_programacion": 0,
            "porcentaje_tecnica": 0,
            "alerta_tecnica": False,
...
# CAT├üLOGOS (selects del formulario de programaci├│n)
# ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
async def listar_ambientes() -> list:
    ...
    if not _es_demo():
        ...
        map_jornada_inv = {430120000: "Ma├▒ana", 430120001: "Tarde", 430120002: "Noche"}
...
async def _validar_asignacion_dataverse(datos: dict, excluir_id: str | None = None):
    ...
    if fin_ficha and dt_fin > date.fromisoformat(fin_ficha[:10]):
        raise HTTPException(status_code=409, detail=f"La ficha {codigo_actual} termina su etapa lectiva el {fin_ficha[:10]}: no se puede programar despu├®s.")
    ...
    for otra in asignaciones_cruce:
        ...
        if (otra.get("_cr6a3_fichaid_value") or "").lower() == (ficha_id or "").lower():
            raise HTTPException(
                status_code=409,
                detail=f"La ficha {codigo_actual} ya tiene una programaci├│n que se cruza entre el {otra.get('cr6a3_fecha_inicio')} y el {otra.get('cr6a3_fecha_fin')}."
            )
        ...
        if (otra.get("_cr6a3_instructorid_value") or "").lower() == (instructor_id or "").lower():
            raise HTTPException(
                status_code=409,
                detail=f"El instructor ya est├í ocupado en la jornada {jornada_texto_actual} con otra ficha del {otra.get('cr6a3_fecha_inicio')} al {otra.get('cr6a3_fecha_fin')}."
            )
        ...
        if (otra.get("_cr6a3_ambienteid_value") or "").lower() == (ambiente_id or "").lower():
            raise HTTPException(
                status_code=409,
                detail=f"El ambiente ya est├í ocupado en la jornada {jornada_texto_actual} con otra ficha del {otra.get('cr6a3_fecha_inicio')} al {otra.get('cr6a3_fecha_fin')}."
            )
...
async def crear_asignacion(datos: dict) -> dict:
    if not _es_demo():
        ...
        # Nota: Dataverse suele requerir el nombre l├│gico en plural para los @odata.bind. 
        # Ejemplo: cr6a3_instructors para cr6a3_instructor.
        payload = {
            "cr6a3_nombre": f"Asignaci├│n Ficha",
...
async def actualizar_asignacion(asignacion_id: str, datos: dict) -> dict:
    """Edita una asignaci├│n revalidando todas las reglas (se excluye a s├¡ misma)."""
    if not _es_demo():
        ...
        if not actual_raw:
            raise HTTPException(status_code=404, detail="La asignaci├│n no existe en Dataverse.")
        ...
        return {"id": asignacion_id, "mensaje": "Asignaci├│n actualizada en Dataverse"}
...
async def obtener_indicadores(mes: str | None = None) -> dict:
    """`mes` en formato YYYY-MM; por defecto el mes actual de Bogot├í."""
    if not _es_demo():
        return {
            "modo_demo": False,
    ...
    if not _es_demo():
        ...
        for p in programas_db:
            programas_mapeados.append({
                ...
                "nivel": p.get("cr6a3_nivel", "T├®cnico"),
                "total_horas": 0, # Podr├¡amos traer las competencias para sumar, pero en el listado b├ísico puede omitirse o calcularse luego
                "competencias": []
            })
...
async def crear_programa(datos: dict) -> dict:
    """Registra un programa en el cat├ílogo (nombre + versi├│n ├║nicos)."""
    if not _es_demo():
        from services.dataverse import crear_registro_dataverse
        ...
        # Extraer din├ímicamente la llave primaria (puede llamarse cr6a3_programaid o cr6a3_programasid)
        ...
        if not programa_id:
            print(f"Error: Dataverse no devolvi├│ un ID reconocido. Respuesta: {programa_creado}")
            raise HTTPException(status_code=500, detail="El programa se cre├│ pero no se pudo obtener su ID interno.")
...
async def crear_ficha(datos: dict) -> dict:
    ...
    elegido en el cat├ílogo (as├¡ la ficha nace con su diagn├│stico registrado)."""
    if not _es_demo():
        ...
        try:
            ...
            raise HTTPException(status_code=404, detail="El programa seleccionado no existe en el cat├ílogo.")
        ...
        try:
            ...
            if "duplicate" in str(e.detail).lower() or e.status_code == 409:
                raise HTTPException(status_code=409, detail=f"Ya existe una ficha con el c├│digo {datos['codigo']}. Verifique el n├║mero en Sof├¡a Plus.")
            raise e
            
        # 3. Obtener competencias del cat├ílogo
...
async def calendario_instructor(instructor_id: str | None = None, correo: str | None = None) -> dict:
    """Programaci├│n completa de un instructor (por id o por correo institucional):
    ...
    if not instructor:
        raise HTTPException(
            status_code=404,
            detail="No se encontr├│ un instructor con esos datos en la programaci├│n de tituladas.",
        )
...
# ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
# RESPALDO DE ARCHIVOS (los Excel hist├│ricos de cada ficha, solo lectura)
# ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
async def agregar_archivo_ficha(ficha_id: str, nombre: str, contenido: bytes) -> list:
    """Guarda un archivo hist├│rico de la ficha (no se modifica: solo respaldo
...
async def actualizar_diagnostico(ficha_id: str, competencias: list) -> dict:
    ...
    async with _demo_lock:
        ...
        if any(f["codigo"] == datos["codigo"].strip() for f in db["fichas"]):
            raise HTTPException(
                status_code=409,
                detail=f"Ya existe una ficha con el c├│digo {datos['codigo']}. Verifique el n├║mero en Sof├¡a Plus.",
            )
        programa = _buscar(db["programas"], datos["programa_id"], "El programa del cat├ílogo")
        ...
        ficha = {
            ...
            # El diagn├│stico nace copiado del cat├ílogo (con ids propios de la ficha)
            "competencias": [
