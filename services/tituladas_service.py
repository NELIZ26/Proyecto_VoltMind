...
async def consultar_disponibilidad(
    ...
    for i in db["instructores"]:
        if i.get("fin_contrato") and fin > _fecha(i["fin_contrato"]):
            ...
            if cruces:
                ...
                estado = "ocupado"
                detalle = (f"Ocupado con la ficha {ficha_cruce.get('codigo', '')} "
                           f"({ficha_cruce.get('programa', '')}) del "
                           f"{_formatear(cruce['fecha_inicio'])} al {_formatear(cruce['fecha_fin'])}")
            else:
                estado, detalle = "disponible", "Disponible en todo el rango"

        # Mini-horario del instructor: TODO lo que tiene en el rango, en cualquier
        # jornada (el "cuadrito" que la coordinadora consulta al pasar el cursor)
        ocupaciones = []
        for a in sorted(db["asignaciones"], key=lambda x: x["fecha_inicio"]):
            if a["instructor_id"] != i["id"] or a["id"] == excluir_asignacion:
                continue
            if not _solapan(inicio, fin, _fecha(a["fecha_inicio"]), _fecha(a["fecha_fin"])):
                continue
            ficha_a = fichas_por_id.get(a["ficha_id"], {})
            competencia = next(
                (c["nombre"] for c in ficha_a.get("competencias", []) if c["id"] == a["competencia_id"]), "")
            ocupaciones.append({
        ...
        instructores.append({**_resumen_instructor(i), "tipo_vinculacion": i["tipo_vinculacion"],
                             "perfil": i.get("perfil", []), "estado": estado, "detalle": detalle,
    ...
    for am in db["ambientes"]:
        ...
        if cruces:
            ...
            ficha_cruce = fichas_por_id.get(cruce["ficha_id"], {})
            estado = "ocupado"
            detalle = (f"Reservado por la ficha {ficha_cruce.get('codigo', '')} del "
                       f"{_formatear(cruce['fecha_inicio'])} al {_formatear(cruce['fecha_fin'])}")
        else:
            estado, detalle = "disponible", "Disponible en todo el rango"
        ...
        ambientes.append({"id": am["id"], "nombre": am["nombre"], "sede": am["sede"],
                          "capacidad": am["capacidad"], "tipo": am["tipo"],
                          "capacidad_suficiente": capacidad_ok, "sede_coincide": sede_coincide,
